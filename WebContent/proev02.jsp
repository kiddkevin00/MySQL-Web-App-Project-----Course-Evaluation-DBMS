...
<%
	...
	try {
		...
		rset5 = stmt
				.executeQuery("select P.pname, C.cname, G.creditforprofessorofcourse from givecredittoprofessor G,professorteachunder P,coursein C where  P.pid=G.pid AND P.pid='005' AND C.cid='014' AND C.cid=G.cid");

		conn6 = ods.getConnection();
		stmt = conn6.createStatement();
		rset6 = stmt
				.executeQuery("select P.pname, C.cname, G.creditforprofessorofcourse from givecredittoprofessor G,professorteachunder P,coursein C where  P.pid=G.pid AND P.pid='006' AND C.cid='015' AND C.cid=G.cid");

		conn7 = ods.getConnection();
		stmt = conn7.createStatement();
		rset7 = stmt
				.executeQuery("select P.pname, C.cname, G.creditforprofessorofcourse from givecredittoprofessor G,professorteachunder P,coursein C where  P.pid=G.pid AND P.pid='007' AND C.cid='016' AND C.cid=G.cid");
...
<body
	...
	<div align="center">
		...
		<form name="registerForm" action="register" method="post">
			<input type="button" title="Chemistry" value="OLAES" id="pro5">
			<input type="button" title="Math" value="ALDAPE" id="pro6"> <input
				type="button" title="Neuro" value="SHARMA" id="pro7">
		</form>
	</div>
	...
	<div align="center" style="border: 5px solid brown">
		...
		<a href="http://localhost:9080/CS4111/proavg2.jsp">Check the
			average evaluation of each professor</a> <br> <a
			href="http://localhost:9080/CS4111/proev01.jsp"> Previous page /</a>
		<a href="http://localhost:9080/CS4111/proev03.jsp"> / Find more </a>
	</div>
	...
	$('#result5').html('<%if (rset5 != null) {
				while (rset5.next()) {
					...
					out.print("<td>" + rset5.getString("pname") + "</td><td>"
							...
							+ rset5.getString("creditforprofessorofcourse")
							+ "</td>");
					out.print("</tr>");
				}
			} else {
	...
	$('#result6').html('<%if (rset6 != null) {
				while (rset6.next()) {
					...
					out.print("<td>" + rset6.getString("pname") + "</td><td>"
							...
							+ rset6.getString("creditforprofessorofcourse")
							+ "</td>");
					out.print("</tr>");
				}
			} else {
	...
	$('#result7').html('<%if (rset7 != null) {
				while (rset7.next()) {
					...
					out.print("<td>" + rset7.getString("pname") + "</td><td>"
							...
							+ rset7.getString("creditforprofessorofcourse")
							+ "</td>");
					out.print("</tr>");
				}
			} else {
