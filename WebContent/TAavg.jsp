...
<%
	...
	try {
		...
		rset = stmt
				.executeQuery("select C.cname, G.TAid,AVG(G.creditforTAofcourse) AS avg from givecredittoTA G,coursein C,TAassist T where G.TAid=T.TAid AND C.cid=G.cid AND T.TAid='01' AND C.cid='010' group by C.cname, G.TAid");

		conn2 = ods.getConnection();
		stmt = conn2.createStatement();
		rset2 = stmt
				.executeQuery("select C.cname, G.TAid,AVG(G.creditforTAofcourse) AS avg from givecredittoTA G,coursein C,TAassist T where G.TAid=T.TAid AND C.cid=G.cid AND T.TAid='02' AND C.cid='011' group by C.cname, G.TAid");

		conn3 = ods.getConnection();
		stmt = conn3.createStatement();
		rset3 = stmt
				.executeQuery("select C.cname, G.TAid,AVG(G.creditforTAofcourse) AS avg from givecredittoTA G,coursein C,TAassist T where G.TAid=T.TAid AND C.cid=G.cid AND T.TAid='03' AND C.cid='012' group by C.cname, G.TAid");
...
<body
	...
	<div align="center">
		<h1>Click the following TA's ID to check the AVERAGE evaluation
			of each course they assist</h1>
	...
	<div align="center" style="border: 5px solid brown">
		...
		<a href="http://localhost:9080/CS4111/ta1.jsp">Go back</a>
	</div>
